...
class TestPurchaseProductPackagingDefault(TransactionCase):
    ...
    def test_product_change_sets_default_packaging(self):
        ...
        self.assertEqual(line.price_subtotal, 120.0)
    ...
    def test_product_qty_updates_pack_qty_without_rounding_away_fraction(self):
        line = self.env["purchase.order.line"].new(
            {
                "order_id": self.purchase_order.id,
                "product_id": self.product.id,
        ...
        self.assertAlmostEqual(line.product_packaging_qty, 13.0 / 12.0, places=6)

    def test_pack_qty_drives_subtotal_via_pack_price(self):
        line = self.env["purchase.order.line"].new(
            {
                "order_id": self.purchase_order.id,
                "product_id": self.product.id,
        ...
        line.onchange_product_id()
        line.pack_price = 100.0
        line._inverse_pack_price()
        line._compute_amount()

        self.assertEqual(line.product_packaging_qty, 1.0)
        self.assertEqual(line.product_qty, 12.0)
        self.assertAlmostEqual(line.price_unit, 100.0 / 12.0, places=6)
